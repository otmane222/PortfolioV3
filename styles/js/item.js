...
export class Item {
    ...
    constructor(DOM_el) {
        ...
        this.calculateSizePosition(this.DOM.deco );
    ...
    calculateSizePosition(DIV) {
        // current scroll
        // console.log(DIV)
        // const rect = DIV.getBoundingClientRect();
        this.scrollVal = {x: DIV?.x , y: DIV?.y};
        // size/position
    ...
    initEvents() {
        ...
        this.DOM.el.addEventListener('mousemove', () => {
            ...
            this.randomString = getRandomString(100);
        });
    ...
    // JavaScript part (main render function and loop)
    render(isFirstTick) {
        // Clear requestId for the next frame
        this.requestId = undefined;
        
        // Calculate the difference between the current scroll position and the stored one
        const scrollDiff = {
        ...
        // Calculate the new translation values based on the mouse position, scroll difference, and element's position
        ...
        // If it's the first animation tick, set the previous values to be the same as the current ones
        if ( isFirstTick ) {
            this.renderedStyles['x'].previous = this.renderedStyles['x'].current;
            this.renderedStyles['y'].previous = this.renderedStyles['y'].current;
        }

        // Update the previous value to be a linear interpolation between the previous and current values
        for (const key in this.renderedStyles ) {
            this.renderedStyles[key].previous = lerp(this.renderedStyles[key].previous, this.renderedStyles[key].current, this.renderedStyles[key].amt);
        }

        // Sync the rendered styles with CSS custom properties for x and y positions
        gsap.set(this.DOM.el, {
            '--x': this.renderedStyles['x'].previous,
            '--y': this.renderedStyles['y'].previous
        });

        // Set the deco element's innerHTML to the random string (optional)
    ...
    // The loopRender function (you can call it via requestAnimationFrame or as needed)
    loopRender() {
        this.requestId = requestAnimationFrame(() => {
            this.render(false); // Pass false if it's not the first tick
        });
    }
}